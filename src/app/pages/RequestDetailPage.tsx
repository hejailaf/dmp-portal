...
export function RequestDetailPage({ id }: { id: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {banner && (
        <p className="rounded-md border border-destructive/40 bg-red-50 p-3 text-sm text-destructive">{banner}</p>
      )}
      {req.status === 'Rejected' && req.rejectReason && (
        <p className="rounded-md border border-destructive/40 bg-red-50 p-3 text-sm">
